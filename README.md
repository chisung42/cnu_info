...
새 공지를 전송할 때는 제목·링크 메시지 뒤에 공지용으로 생성된 이미지도 텔레그램 앨범으로 전송합니다. 이미지는 한 앨범에 최대 10장씩 나뉩니다.
그 다음에는 대시보드의 `본문 복사`와 완전히 같은 업로드용 텍스트(게시판 헤더·본문·해시태그)를 별도 메시지로 보내므로, 텔레그램에서 바로 전체 복사해 업로드할 수 있습니다. 본문이 4,096자를 넘으면 문단 또는 줄바꿈 경계에서 나뉩니다.
